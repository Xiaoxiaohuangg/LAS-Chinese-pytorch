...
class AllDataset(Dataset):
    def __init__(self, X, Y, max_timestep, max_label_len, batch_size, mean_val, std_val, data_type, transform = None,target_transform = None):
...
#                 self.max_label_len = len(self.Y[index])+50
            #pdb.set_trace()
...
#             img_features = normalize(img_features, self.mean_val, self.std_val)
            img_features = set_type(img_features, self.data_type)
            X_.append(img_features)
            if index % self.batch_size == 0:
                self.max_timestep = min(2000,int((total_frames/8)+20)*8)
...
def create_dataloader(X, Y, max_timestep, max_label_len, batch_size, mean_val, std_val, data_type, transform, target_transform, shuffle,  **kwargs):

    return DataLoader(AllDataset(X,Y,max_timestep,max_label_len,batch_size,mean_val,std_val,data_type,transform, target_transform),
